    ...
    private void executeMove(Pawn pawn, int[] dest, boolean animated) {
        ...
        if (dest != null && board.canReachCell(dest[0], dest[1])) {
            ...
            if (!animated) { // move by clicks
                ...
                // only generate the captures actions, otherwise when dropped the pawn will briefly appear on its starting square before teleporting
                board.moveElement(pawn, dest[0], dest[1]);
                ActionList actions = tablutControl.genCapturesAction(model, pawn, board, dest[0], dest[1]);
                ActionPlayer player = new ActionPlayer(model, control, actions);
                player.start();
            }
